 ...
 * `server/glitchtip/`. Initialized first thing in the flavor Application (TelemetryApplication) so a
 * crash during startup (the ones that hurt most) is still captured. telemetry source set
 * (github/playstore) only — the FOSS fdroid build never links the Sentry SDK.
 *
